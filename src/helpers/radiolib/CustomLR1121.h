  ...
  public:
    ...
    size_t getPacketLength(bool update) override {
      ...
      if (getIrqStatus() & RADIOLIB_LR11X0_IRQ_HEADER_ERR) {
        MESH_DEBUG_PRINTLN("LR1121: header error, resetting RX path");
        recoverReceivePath();
        return 0;
      }
      return LR1121::getPacketLength(update);
    }

    int16_t readData(uint8_t* data, size_t len) override {
      int16_t state = LR1121::readData(data, len);
      ...
      if (mesh::hasLR1110RxBufferShiftSignature(data, len)) {
        MESH_DEBUG_PRINTLN("LR1121: four-byte RX shift detected, dropping packet");
        recoverReceivePath();
